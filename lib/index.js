...
module.exports = function (options) {
  ...
  return function (files, metalsmith, done) {
    ...
    // array of gatherers
    const gatherers = options.patterns.reduce(function (acc, pattern) {
      acc.push(
    ...
    async.parallel(gatherers, function (error, filesContents) {
      ...
      filesContents.push('')
      const contents = new Buffer(filesContents.join(options.insertNewline ? options.insertNewline : ''))
...
function gathererFromSearchPaths (rootPath, searchPaths, pattern) {
  return function gatherer (done) {
    ...
    }, function (error, filesContents) {
      if (error) {
        return done(error)
      }
      filesContents = Array.prototype.concat.apply([], filesContents)
      return done(null, filesContents)
    })
  }
}
